...
function ccd()
{
    ...
    &es.exe $arguments -sort-date-modified-descending -name -dm -size | invoke-fzf | % {if ((get-item $_) -is [system.io.directoryinfo])
        {cd $_
        } else
        {cd (split-path -path $_)
        }}
}
...
{
    write-host $args
    &es.exe $args | invoke-fzf | % {if ((get-item $_) -is [system.io.directoryinfo])
        {cd $_
        } else
        {cd (split-path -path $_)
        }}
...
function zf
{
    ...
    $dir = zoxide query -l | fzf @fzfArgs
...
function Invoke-P4ReconcileAndCommit
{
    ...
    try
    {
        ...
        Start-Process -FilePath "p4vc" -ArgumentList "pendingchanges"
    } catch
    {
        Write-Error "操作失败: $_"
    } finally
